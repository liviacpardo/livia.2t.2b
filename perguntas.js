criaCartao(
    'Ferrari',
    'Quem são os pilotos atuais da equipe Ferrari?',
    'Lewis Hamilton, Charles Leclerc.'
)

criaCartao(
    'Ayrton Senna',
    'Quando morreu Ayton Senna?',
    '1 de maio de 1994.'
)

criaCartao(
    'Circuítos',
    'Quantos circuítos possui a temporada de 2025?',
    '24 circuítos, incluindo autódrmos e corridas de rua.'
)

criaCartao(
    'Mc Laren',
    'Quantos títulos a Mc Laren possui atualmente (maio 2025)?',
    ' 9 títulos mundiais de construtores e 12 títulos mundiais de pilotos.'
)

criaCartao(
    'Lewis Hamilton',
    'Qantos títulos pela F1 possui o piloto Lewis Hamilton?',
    '7 títulos mundiais,  sendo eles: 2008, 2014, 2015, 2017, 2018, 2019 e 2020'
)

criaCartao(
    'Michael Schumacher',
    'Qual foi a ultima equipe e o ultímo circuíto que Michael Schumacher correu plea ultíma vez antes de seu acidente?',
    'Equipe Mercedes, no circuíto de Interlagos, no Brasil, em 2012.'
)

criaCartao(
    'Campeão',
    'Quem foi o ultímo campeão mundial de Formúla 1 (2024)?',
    'Max Verstappen.'
)

criaCartao(
    'Pilotos',
    'Quantos pilotos disputam disputam atualmente (2025) o campeonato automobilístico de Fórmula 1?',
    '20 pilotos.'
)

criaCartao(
    'Equipes',
    'Quantas e quais são as equipes atuais (2025)?',
    '10 equipes, Aston Martin, BWT Alpine, McLaren, Mercedes, Haas, Red Bull, Scuderia Ferrari, Stake Kick Sauber, Visa Cash App Racing Bulls, Williams.'
)
criaCartao(
    'Fórmula 1',
    'Quantos anos tem a Fórmula 1 e onde foi a primeira corrida?',
    'Completa 75 anos neste ano de 2025. A primeira corrida oficial ocorreu em Silverstone, Inglaterra.'
)